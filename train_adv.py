...
def trainClassifier(model, train_loader, test_loader, autoencoder, args, use_cuda=True):    
    if use_cuda:
        model = model.cuda()
    ...
    for epoch in range(args['num_epoch']):
        ...
        for x, target in tqdm(train_loader):            
            ...
            if args['clean']:
                ...
                use = (components > args.thrd)
                x_adv_new = x_adv[use,:,:,:]
            
            loss = criterion(model(x_adv_new),target)
...
def main(args):
    ...
    model = trainClassifier(model, train_loader, test_loader, autoencoder, args, use_cuda=use_cuda) 
...
if __name__ == "__main__":
    ...
    parser.add_argument("-f", "--file_name", default="cifar10_clean")
    parser.add_argument("-l", "--lr", type=float, default=1e-4)
    parser.add_argument('--thrd', type=float, default=0.05, help='M:, C:, S:, T:, thrd for tangent components') 
    parser.add_argument("--init", default='cifar10_clean')
    parser.add_argument("--weight_decay", type=float, default=1e-4)
    parser.add_argument("--root", default=r'D:\yaoli')
    parser.add_argument("--clean", action="store_false", default=True, help='if true, clean training')
